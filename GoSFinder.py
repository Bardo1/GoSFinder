...
try:    
    ...
    for i in array:
        try:
            ...
            if checkurl == 200:
                print Fore.GREEN + website+i, " " * (length - length_url) + "[+]FOUND[+]"
                continue
        except urllib2.URLError, checkurl:
            length_url = len(website+ i)
            if checkurl == 400:
                print website+i," " * (length - length_url) + "[+]NOT FOUND[+]"
            else:
                print Fore.RED + website+i, " " * (length - length_url) + "[-]ERROR[-]" + Style.RESET_ALL
